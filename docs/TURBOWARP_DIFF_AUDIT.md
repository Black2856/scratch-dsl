...
| `goToBack` の追加引数 | `setDrawableOrder(..., SPRITE_LAYER)` | `setDrawableOrder(..., SPRITE_LAYER, false)`(TW renderer 拡張シグネチャ) | 本プロジェクトは自作renderを持たない(視覚は実VM)。layer順はmodelで保持 | `default`(差分は内部) | TW `rendered-target.js:872 goToBack` |
...
本プロジェクトの `src/runtime/fencing.ts` は前者(renderer フェンス)を純関数で再現している。
...
本プロジェクトは `RendererPort` を seam として持つ（自作Canvas描画レイヤは削除済み）。ここでは **VM↔renderer の seam** に限定して比較する。
...
| getFencedPositionOfDrawable | renderer 共通(setXY 用) | 同一(VM 側は §1 のトグルで呼出制御) | 本プロジェクトは `fencing.ts` で再現済み | `default` | 本 `src/runtime/fencing.ts`、§1 参照 |
| bounds 計算(fast/tight) | renderer の getFastBounds/getBounds | TW renderer は同等 API(本書範囲外の最適化差あり) | 本プロジェクトは fast(矩形 AABB)を `computeLocalBounds` で再現 | `default` | 本 `src/runtime/fencing.ts computeLocalBounds` |
...
| 項目 | Scratch v14.1.0挙動 | TurboWarp挙動 | 本プロジェクトへの影響 | 採用判断 | 参照ファイルと関数名 |
|------|--------------------|---------------|----------------------|----------|---------------------|
| ステージ寸法 | 480×360 固定(`STAGE_WIDTH`/`STAGE_HEIGHT` 定数のみ) | `runtime.stageWidth/stageHeight` インスタンス値、`setStageSize(w,h)` で変更可 | 本プロジェクトは 480×360 固定(`coordinates.ts`)。可変化は大規模変更 | `default`(480×360 固定) | 公式 `runtime.js:418 STAGE_WIDTH`(=480)<br>TW `runtime.js:453-454, 2704 setStageSize`<br>本 `src/runtime/coordinates.ts:8-9` |
| ステージサイズ変更時のモニタ移動 | なし | 中心基準でモニタ位置を補正 | 可変化しないため無関係 | `unsupported` | TW `runtime.js:2710-2722 setStageSize` |
| renderer への伝播 | 固定 | `renderer.setStageSize(±w/2, ±h/2)` | 本プロジェクトは自作renderを持たない(実VMは480×360固定) | `unsupported` | TW `runtime.js:2726-2733` |
...
- **原因(本プロジェクトの差分):** `src/runtime/fencing.ts fencePosition` の y 軸
...
| 項目 | Scratch v14.1.0挙動 | 本プロジェクト(修正前) | 採用判断 | 参照 |
|----|----|----|----|----|
| fencing の y エッジ選択 | 下→`top` / 上→`bottom`(x と同型) | top/bottom が逆 | `default`(公式挙動を採用・**修正済み**) | 公式 `scratch-render getFencedPositionOfDrawable`<br>本: `src/runtime/fencing.ts fencePosition` |

## 14. pen の解像度・フィルタリング (歴史的記録)

> **注:** 本節で言及していた自作 `CanvasRenderer` は削除済み。視覚出力は実 Scratch VM
> (`scratch-render`) が担う。pen の解像度差は `npm run preview` / `npm run shot` による
> 実機比較で確認する。

正本 Scratch / scratch-render における pen の挙動:

- pen レイヤ(`PenSkin`)のテクスチャは `gl.NEAREST` サンプリングで、サイズは
  native size(stage × devicePixelRatio)で確保される。Scratch は **device 解像度**で
  pen を描くため、拡大表示でも鮮明かつ細かく、半透明エッジ(AA)も保たれる。
  スプライトは LINEAR 描画。
- 本プロジェクトでは `scratch-render` を直接利用するため、この挙動は正本と一致する。

| 項目 | Scratch v14.1.0挙動 | 本プロジェクト | 採用判断 | 参照 |
|----|----|----|----|----|
| 描画解像度 | device 解像度(stage × DPR) | 実 `scratch-render` に委任 | `default` | 公式 `scratch-render/src/PenSkin.js`, `RenderWebGL.js`(`pixelRatio`) |
