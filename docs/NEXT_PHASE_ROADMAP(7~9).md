...
fixture素材は`workspace/test-project/`から使用し、外部素材を無断で
追加しない。
...
ローカル作品固有のDSL、asset manifest、生成物を
`workspace/projects/<name>/`へ集約する。共通Runtime、Renderer、
AssetManager、SoundManager、serializer、preview、CLIは本体側へ置く。

```text
workspace/projects/<name>/project.ts
